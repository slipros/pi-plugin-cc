...
| Credentials | Never enter the container: a run-scoped proxy on the host holds the real key and the agent gets a token that dies with the run. Works for custom providers and for the ones pi ships with — their addresses come from the model catalogue beside the `pi` binary |
