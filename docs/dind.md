...
| `--security-opt systempaths=unconfined` | the nested runc mounts its own `/proc`, and the kernel only allows a procfs mount inside a user namespace when an already mounted procfs is fully visible. Docker's masked paths (`/proc/kcore` and friends under tmpfs, `/proc/sys` read-only) break that, so every container started inside the sandbox dies with `error mounting "proc" to rootfs … operation not permitted` |
...
**Not needed:** `apparmor=unconfined` (AppArmor is not active on Docker Desktop; on native Linux with AppArmor enabled the `docker-default` profile will likely report some mount operations on top of seccomp — there it probably would be), `--device /dev/fuse` (overlay2 works natively on top of a volume), `--privileged`.
...
**Portability.** The image is built for the host's uid (`RUNTIME_UID`/`RUNTIME_GID` are passed from `process.getuid()`): for uid ≠ 1000 a user and a subuid range are created, otherwise RootlessKit does not start. Changing user means rebuilding the image (`sandbox build agent`). An image built without those args — `docker build` run by hand, say — carries the 1000 default: `/run/user/<uid>` then belongs to nobody the run can write, the entrypoint warns once on stderr and falls through to a plain `exec pi`, and the run looks healthy right up to the integration test that finds no daemon. `sandbox build agent` through the companion always passes the caller's own uid.
