...
- Commit identity: `--git-name` and `--git-email` (both, or neither) make the agent commit as itself instead of as the user. Pass them when the user wants agent commits distinguishable in history — they are the only layer that outranks the user's own gitconfig, which a preset's `git` block does not.
