...
Отчёт даёт не только токены: `ok` — доля прогонов, дошедших до конца; `tok/s` — скорость модели; `p50`/`p90` — длительность прогона; `tools`/`err` — вызовы инструментов и ошибки в них.
