...
- Каталог агента — отдельный docker-volume, поэтому хостовые настройки, сессии и установленные пакеты pi внутрь не попадают. Учётные данные подкладываются одним файлом `auth.json` в режиме read-only.
